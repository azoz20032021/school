        ...
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
            ...
            >
                <div className="flex flex-col items-center mb-6">
                    ...
                    <h1 className="text-xl font-black text-slate-800">{t('متابعة حالة الطلب')}</h1>
                    <p className="text-slate-500 text-xs font-medium mt-1">{t('أدخل رقم المتابعة الذي استلمته عند التسجيل')}</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    >
                        ...
                        {loading ? t('جاري البحث...') : t('استعلام')}
                    </button>
                </form>
                ...
                {result && view && Icon && (
                    ...
                    >
                        ...
                        <div className="mt-4 pt-4 border-t border-current/10 space-y-1.5 text-xs">
                            <p><span className="opacity-70">{t('الاسم:')}</span> <span className="font-bold">{result.full_name}</span></p>
                            {result.requested_class_name && (
                                <p><span className="opacity-70">{t('الصف:')}</span> <span className="font-bold">{result.requested_class_name}</span></p>
                            )}
                            {result.status === 'rejected' && result.rejection_reason && (
                                <p><span className="opacity-70">{t('السبب:')}</span> <span className="font-bold">{result.rejection_reason}</span></p>
                            )}
                        </div>

                        {result.status === 'approved' && result.assigned_uid && (
                            <div className="mt-4 bg-white/70 rounded-xl p-4 text-center">
                                <p className="text-[10px] font-bold uppercase tracking-widest opacity-70 mb-1">{t('رقمك التعريفي')}</p>
                ...
                <div className="mt-6 text-center">
                    <Link to="/login" className="text-slate-500 text-sm hover:underline">{t('العودة لصفحة الدخول')}</Link>
                </div>
            </motion.div>
        </div>
    );
